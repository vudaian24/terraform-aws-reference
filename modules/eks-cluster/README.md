...
**Status: implemented.** Alternative compute path to `modules/ecs-service` — a
given env wires one, not both. Kubernetes-level resources (Deployments/Services/Ingress) are out of scope
...
- `node_security_group_id` (for `modules/database`/`modules/cache` ingress
  rules on the EKS compute path — mirrors what `modules/ecs-service`'s
  `task_security_group_id` gives the ECS path)
